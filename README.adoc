...
A Personal Access Token (PAT) with read-only permission is required to read the alerts from the public repository. This token is used in the YAML file, which pushes the alerts to {PRODUCT}.
